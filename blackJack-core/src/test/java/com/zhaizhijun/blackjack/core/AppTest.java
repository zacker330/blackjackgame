...
public class AppTest {
    ...
    public void testName() throws Exception {
        ...
        DealerProxy dealerProxy = new DealerProxy("A");

        PlayerProxy playerProxyA = new PlayerProxy("B", new BigDecimal(20));

        PlayerProxy playerProxyB = new PlayerProxy("c", new BigDecimal(30));
    ...
    public void test_dealer_player_comparePoint() throws NoSuchFieldException, IllegalAccessException {
        ...
        PlayerProxy player = new PlayerProxy("player", new BigDecimal(20));

        DealerProxy dealer = new DealerProxy("dealer");
        ...
        tables.addPlayerProxy(player);

        tables.setDealerProxy(dealer);

        when(dealerRobot.deal()).thenReturn(Card.create(CardColor.CLUB, CardFace._6));

        tables.opening();

        setPartProxyHeadCard(player, createMock13PointHeadCard());
        player.flushCardCategory();

        setPartProxyHeadCard(dealer, createMock13PointHeadCard());
        dealer.flushCardCategory();


        assert player.getActions().contains(new HitAction());

        tables.hit(player);

        assert player.getSumPoint() == 19;

        assert player.actionsContains(new StandAction());
        ...
        assert dealer.getSumPoint() == 20;

        assert tables.betWinnerLoserCalculate(player).getWinner().equals(Winner.DEALER);
    ...
    public void test_buy_insurance() throws NoSuchFieldException, IllegalAccessException {
        ...
        PlayerProxy playerProxy = new PlayerProxy("player", new BigDecimal(20));


        tables.addPlayerProxy(playerProxy);

        DealerProxy dealerProxy = new DealerProxy("dealer");
        tables.setDealerProxy(dealerProxy);
    ...
    public void test_auto_play() {
        ...
        PlayerProxy playerProxy = new PlayerProxy("playerA", new BigDecimal(20));
        PlayerProxy playerProxy1 = new PlayerProxy("playerB", new BigDecimal(40));

        tables.addPlayerProxy(playerProxy);
        LOGGER.info("玩家 {} 下注一门，注大小为： {} ", playerProxy.getPlayer(), playerProxy.getBetMoney().intValue());

        tables.addPlayerProxy(playerProxy1);
        LOGGER.info("玩家 {} 下注一门，注大小为： {} ", playerProxy1.getPlayer(), playerProxy1.getBetMoney().intValue());
        ...
        LOGGER.info("{} 得到牌:{}", dealerProxy.getPlayer(), dealerProxy.getHeadCard());
        LOGGER.info("{} 得到牌:{}", playerProxy.getPlayer(), playerProxy.getHeadCard());
        LOGGER.info("{} 得到牌:{}", playerProxy1.getPlayer(), playerProxy1.getHeadCard());
